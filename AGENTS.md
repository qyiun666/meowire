...
- **近三期靶子**：① 挂起-恢复这条主线从 v1.3.0/1.3.2 的 ask 挂起走到 v1.3.8 的「膜守循环两侧 + 挂起成因入线」（`Verdict` 三态在 `Allow` 与 `Emit` 两侧同形，Session wire v3 带上归属 cell、按名编码的 waitKind、被扣住的草稿与未答请求），主线已闭合；② 当前重心是**接线闭环**：宿主只实现器官就能组出多 agent 内核——入站侧收件箱归 cell（间隙点排空，蓝图 G3）+ 路由表由 `Resolve(agents...)` 从 agent 列表建出，出站侧委托是 `Effect.Send` 的返回值（cell 铸 ID、盖 `Organs.Colony`，蓝图 G2，唯一的可选器官）而非宿主手里的 synapse，回程作答挂在调用终态上（六个 `TaskStatus` 各一个写入者），配对归内核、是否续跑归宿主（`Resumptions`/`Ack`，框架永不自行 Resume）；剩下的接线是器官组合器/生命周期时点与事件流 wire 编码；`Prompt.Reflection` 槽位与 `CycleOutcome` 终态分类仍停在 "lite"，语义还没定齐；③ 公开面收敛（八钩子必填 + `OnCycleEnd` 终态 + 蓝图独占可换槽名 `WirePoint.Slot` + 预算端口覆盖两条累积轨 + 记忆端口 P7 + 出口裁决归端口不归钩子 P5c + 权重≠连通（传导阈值 `Floor` 断流、`Prune` 的 `weightFloor` 删边，两个阈值不合并）+ 投递时刻与阈值都归图、skill 与 Agent Card 同一份投影），新增一律先问能不能并入现有端口；不能并入且缺的是「框架调用时点」的，升为必填端口，并同批交付 槽名 + Validate 分支 + 同步守卫 三件。
