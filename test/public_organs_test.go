...
// public_organs_test.go — the acceptance face of the kernel: every port is
// implementable with public names alone. internal/ is sealed by the compiler,
// so a contract type missing from api/ makes its port unimplementable outside
// this module — which shows up here as a compile error.
...
type hostThinker struct {
	touch *hostTouch
	round int
}

func (h *hostThinker) Think(_ context.Context, p *meowire.Prompt) (*meowire.Decision, error) {
	h.touch.note("think")
	h.round++
	if h.round == 1 {
		return &meowire.Decision{Text: "working", ToolCalls: []meowire.ToolCall{{ID: "c1", Name: "calc", Args: "1+1"}}}, nil
	}
	return &meowire.Decision{Text: "answer"}, nil
}
...
// exactly as an external host would write them: every port is consulted, in
// loop order, and the cycle ends with the accumulated answer.
func TestPublicSurfaceOrgansAssemble(t *testing.T) {
	touch := &hostTouch{}
	a, err := meowire.New(meowire.Blueprint{
		Organs: meowire.Organs{
			ID:      "public-host",
			Think:   &hostThinker{touch: touch},
