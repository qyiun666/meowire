...
- **`WirePoint.Parallel`** — the blueprint edge carried a boolean nobody read: the loop is serial by
  construction, batch parallelism is a `Config.ParallelActs` decision, and the only consumer was
  `RenderJSON` echoing the field back. A wiring point now says what it touches (`TargetID`), how
  (`Semantics`), whether it is required, and by what name it can be swapped (`Slot`) — nothing the
  host has to keep in step with.
