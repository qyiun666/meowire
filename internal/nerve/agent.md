...
- `DecisionLoop{}.Resume(ctx, *LoopContext, sess Session, response string, yield)`: continues a suspended loop from a Session (EventWaitInput or EventPaused) — for a tool suspension the response is injected as the pending tool's structured result (pending.ID == "" for pause suspensions: nothing is injected, the loop just continues); remaining tools run, round loop resumes from the suspended round (no extra round, no budget during the wait); stream/hooks/guarantees isomorphic with Cycle
...
- `LoopContext{CellID, Identity string, Methods []MethodSpec, Think, Act, Hooks, Sandbox, Budget, Pause, MaxRounds, MaxToolOutput, MaxRetries, ToolTimeout, ToolMaxRetries, ParallelActs, State, Input, Plan, Context, Bounds, ToolResults, PendingReplace, PendingConfig, System, Tools}`: all ports injected via fields
