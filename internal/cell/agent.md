...
- `Cell`: minimal agent kernel
  ...
  - Config: `MaxRounds`, `MaxToolOutput`, `MaxRetries`, `ToolTimeout`, `ToolMaxRetries`, `ParallelActs`, `MaxParallelActs` (zero values use defaults / disabled; ParallelActs=false = strict serial, and a ceiling set while it is off binds nothing — `Validate` says so); `UpdateConfig(cfg)` swaps wholesale at the next Stimulate/Resume and — since v1.3.2 — records a `ConfigAudit{CellID, Old, New}` drained into the next LoopContext and emitted as EventConfig after EventReplace (the config-update counterpart of the Replace audit)
